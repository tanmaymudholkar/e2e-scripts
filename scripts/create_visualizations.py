...
for mass_point_title in mass_points:
    ...
    for clone_index in range(1, 1+settings.values_["visualizations"]["nCopiesPerMassPoint"]):
        ...
        # Run wrapper script
        subprocess.check_call("{er}/scripts/create_visualization_from_pq_sh_wrapper.sh {d}/e2e_visuals/converted_{mpt}_{i}.parquet.0 {o} {ofn}".format(er=e2e_env.e2e_root, d=e2e_env.scratch_area, mpt=mass_point_title, i=clone_index, o=inputArguments.outputFolder, ofn="visualization_{mpt}_{i}.pdf".format(mpt=mass_point_title, i=clone_index)), executable="/bin/bash", shell=True)
